...
- The sidebar is collapsible and defaults to open on desktop while staying closed on mobile; a toggle control in the main header reopens it when hidden.
...
- Each conversation navigates to its own `/journals/:id` URL; new journals launch at `/journals/new` and the marketing homepage is `/`.
